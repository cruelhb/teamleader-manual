...
3. **캡처 마스킹 규칙은 불필요하다.** 올라가는 건 사용법과 기본 규정뿐이라 고객
   ...
   단, 채용요청서 문서의 **작성 예시 캡처에는 실제 입사자 인적사항**(생년월일·학교·전공·
   이전 직장·연봉·면접점수·소속팀)이 들어 있다. 이름·주소·연락처만 가려진 상태이며,
   내가 위험을 알린 뒤 사용자가 **그대로 올리기로 결정**한 것이다. 다시 문제 삼지 말 것.
4. **완전 공개 + 검색엔진 색인 차단**(`robots.txt` + `noindex` 메타). 비밀번호 잠금 없음.
   → **2026-08-25 재검토 시작, 결론 보류.** 사용자가 "직원만 보게 하고 싶다"고 해서 방법을
   정리했으나 더 고민해 보겠다며 미뤘다. 먼저 꺼내지 말고, 사용자가 다시 물으면 아래를 전할 것.
   - **저장소 비공개가 1순위.** 사이트에 비밀번호를 걸어도 공개 저장소에서 그대로 읽힌다.
   - 잠금 방법 두 가지: Pages Functions `_middleware` Basic Auth(공용 비번 1개, 인원 무제한)
     또는 Cloudflare Access(이메일 일회용 코드, 무료 50명, 퇴사자 차단·접속기록).
   - **어느 쪽이든 카톡 미리보기 카드가 죽는다.** 이 사이트 설계의 뿌리와 정면으로 부딪힌다.
     제목·설명(OG)만 열고 본문만 잠그는 절충안을 제시해 둔 상태다.
   - 채용요청서 문서에 직원 인적사항·급여테이블·연봉이 들어가면서 상황이 달라진 것이 발단이다.
